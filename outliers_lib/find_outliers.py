...
def find_outliers_quantile(data, feature, left=0.01, right=0.99):
    """
    Находит выбросы в данных, используя метод отсечения по границе квантилей. 
    Реализовано ручное управление границами отсечения в обе стороны распределения.

    Args:
        data (pandas.DataFrame): набор данных
        feature (str): имя признака, на основе которого происходит поиск выбросов
        left (float, optional): Нижняя граница отсечения (квантиль). По умолчанию равна 0.01.
        right (float, optional): Верхняя граница отсечения (квантиль). По умолчанию равна 0.99.
